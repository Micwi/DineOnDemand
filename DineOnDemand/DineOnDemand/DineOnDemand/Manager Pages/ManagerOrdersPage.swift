...
class ManagerOrdersPage: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func populateArrays() {
        ...
        database.collection("order")
            ...
            .whereField("day_of_order", isEqualTo: selectedDate)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            markAsPickedUpAlert(reservationID: ordersReadyForPickup[indexPath.row]["order_id"] as! String, rowNumb: indexPath.row)
        } else if indexPath.section == 1 {
            // Any Other Order
            ManagerOrderDetails.orderData = self.otherOrders[indexPath.row]
            self.performSegue(withIdentifier: "showOrderDetails", sender: self)
        }
    }
    
    func markAsPickedUpAlert(reservationID: String, rowNumb: Int) {
        ...
        let confirmAction = UIAlertAction(title: "Mark as Picked Up", style: .default) { _ in
            self.changeOrderStatus(reservationID: reservationID)
            self.populateArrays()
        }
        let viewContentsAction = UIAlertAction(title: "View Contents of Order", style: .default) { _ in
            ManagerOrderDetails.orderData = self.ordersReadyForPickup[rowNumb]
            self.performSegue(withIdentifier: "showOrderDetails", sender: self)
        }
    ...
    func changeOrderStatus(reservationID: String) {
        self.database.collection("order").document("order_\(reservationID)").setData([
